      ...
      <div class="content-wrapper">      
        ...
        <div class="box">
                <div class="box-header with-border">
                  <h3 class="box-title">Sales MIS</h3>
                  <a href="<?php echo base_url().'addSale'; ?>" class="fa fa-plus" style="float: right;" data-toggle="tooltip" data-placement="left" title="Add Sale"></a>                  
                </div><!-- /.box-header -->
                <div class="box-body">
                <form action="report/exportToExcelSaleAjax" method="post">
                  ...
                  <table id="saleList" class="table table-bordered table-striped" style="width:100%;">
                    <thead>
                      <tr>
                        <th style="width:13%;">Quote No</th>
                        <th style="width:13%;">Sales Order No</th>
...
function getDataTable(from_date, to_date){
  ...
  var table_news = $('#saleList').DataTable({
    "processing":true,
    "serverSide":true,  
    "order":[], 
    "ajax": {
      url : '<?php echo base_url("report/saleExcelDataAjax"); ?>',
      type: "POST",
...
function JSconfirm(id){
  var text = "Do you really want to delete this record";
    ...
    function(isConfirm){   
      if (isConfirm){   
        window.location = "report/saleDelete/"+id;   
      } else {     
        swal("Sale status is still unchanged.");   
      } 
  });
}
</script>